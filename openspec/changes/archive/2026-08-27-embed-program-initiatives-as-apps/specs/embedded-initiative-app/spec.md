...
系统 MUST 将 embedded Initiative 的宿主契约限制为基础身份、静态应用根、View 边界、生命周期、不透明内部位置和预定义宿主动作。宿主 MUST NOT 为任何消费项目复制业务 schema、解析内部实体关系、读取专项正文、注册专项 renderer App，或保存消费项目专属知识与兼容性资产。
...
- **THEN** 测试只使用仓库内合成的通用 Initiative App fixture
